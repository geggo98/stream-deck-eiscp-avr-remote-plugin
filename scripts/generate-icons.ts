...
/**
 * Also emit the glyph markup as a TypeScript module.
 *
 * The key images above are enough as long as a key shows *only* a glyph. Drawing a
 * glyph on top of the receiver's cover art has to happen at runtime, because the
 * cover is not known until it arrives — and that needs the glyph's markup in the
 * bundle.
 *
 * A generated module rather than reading the SVGs at runtime: `src/` does no file
 * I/O today, Rollup produces a single file with no assets beside it, and this
 * follows the same pattern as the generated command registry. Cost is ~25 short
 * path strings in the bundle.
 */
const GLYPH_MODULE = resolve(PROJECT_ROOT, "src/actions/generated/glyphs.ts");

/**
 * Glyphs the runtime composites that no action declares:
 * `music` is the placeholder for "playing something, but no cover art".
 */
const RUNTIME_GLYPHS = ["music"];

const glyphNames = new Set<string>(RUNTIME_GLYPHS);
for (const spec of DEDICATED_SPECS) {
	const icon: IconSpec = spec.icon;
	glyphNames.add(icon.primary);
	if (icon.onPrimary) glyphNames.add(icon.onPrimary);
	if (icon.badge) glyphNames.add(icon.badge);
}
for (const spec of GENERIC_SPECS) glyphNames.add(spec.iconName);

const glyphEntries = [...glyphNames]
	.sort()
	.map((name) => `\t${JSON.stringify(name)}: ${JSON.stringify(lucideInner(name))},`)
	.join("\n");

mkdirSync(dirname(GLYPH_MODULE), { recursive: true });
writeFileSync(
	GLYPH_MODULE,
...
written += 1;
console.log(`\n  glyph module: ${glyphNames.size} glyphs -> ${GLYPH_MODULE}`);
